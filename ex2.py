...
print 'Theta at lambda = 0.0 is ', theta_opt1


# plot data and training fit for the 6th order polynomial and save it in fig9.pdf

plot_utils.plot_fit(X,y,np.min(X),np.max(X),mu,sigma,theta_opt1,p,'Change in water level (x)','Water flowing out of dam (y)','Polynomial Regression fit with lambda = 0.0 and polynomial features of degree = ' + str(p))
...
#######################################################################
error_combined = error_train + error_val
reg_best = reg_vec[np.argmin(error_combined)]  
error_best_train = error_train[np.argmin(error_combined)]  
error_best_val = error_val[np.argmin(error_combined)]
print 'The best model give error_train',error_best_train, ' and error_val', error_best_val, ' when lamda = ', reg_best
print 'From previous analysis, lamda = 0.3 and number of training examples = 12 give the best model, and then compute the theta for the best model'
reglinear_bestModel = RegularizedLinearReg_SquaredLoss()
theta_bestModel = reglinear_bestModel.train(XX_poly,y,reg=reg_best,num_iters=10000)
print 'The best model give theta = ',theta_bestModel, '. Now use this theta to calculate error on the test set'
error_test_bestModel = (np.dot(np.array([np.dot(XX_poly_test,theta_bestModel)-ytest]),np.array([np.dot(XX_poly_test,theta_bestModel)-ytest]).T)/(2*len(ytest)))[0,0]
print 'The error of the test set using the best model is ', error_test_bestModel
