...
class LinearRegressor_Multi:
    ...
    def normal_equation(self,X,y):
        ...
        theta_n = np.linalg.inv((X.T).dot(X)).dot(X.T).dot(np.array([y]).T)[:,0]
        ###########################################################################
...
class LinearReg_SquaredLoss(LinearRegressor_Multi):
    ...
    def loss (self,X,y):
        ...
        ###########################################################################
        J=np.dot(((np.dot(X,np.array([self.theta]).T))-np.array([y]).T).T,((np.dot(X,np.array([self.theta]).T))-np.array([y]).T))[0]/(2*X.shape[0])                      
        grad=(np.dot(X.T,(np.dot(X,np.array([self.theta]).T))-np.array([y]).T)/X.shape[0]).T[0]
