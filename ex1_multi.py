...
pred_cost = linear_reg3.predict(np.array([np.mean(XX,axis=0)]))*10000
...
pred_cost = np.array([np.mean(XX1,axis=0)]).dot(np.array([theta_n]).T)[0]*10000
print 'For average home in Boston suburbs, we predict a median home value of', pred_cost
...
########################################################################
plot_utils.plot_data(np.array([]),np.array([]),'Number of iterations','Cost J')
for i in learning_rates:
    linear_reg_x = LinearReg_SquaredLoss()
    J_history_x = linear_reg_x.train(XX,y,learning_rate=i,num_iters=5000,verbose=False)
    plt.plot(range(len(J_history_x)),J_history_x,'-',label = 'learing rate is'+' '+str(i) )   
plt.legend()
