...
class RegularizedLinearRegressor_Multi:
    ...
    def predict(self, X):
        ...
        ###########################################################################
        y_pred = np.dot(np.array([self.theta]),X.T)[0]
...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        J = ((X.dot(np.array([theta]).T)-np.array([y]).T).T).dot((X.dot(np.array([theta]).T)-np.array([y]).T))[0,0]/(2*num_examples)+np.array([theta[1:]]).dot(np.array([theta[1:]]).T)[0,0]*reg/(2*num_examples)
    ...
    def grad_loss(self,*args):                                                                          
        ...
        ###########################################################################
        #grad = (((X.T).dot(X.dot(np.array([theta]).T)-np.array([y]).T))[:,0])/num_examples
        #grad[1] = grad[1] + (reg*theta/num_examples)[1]
           
        grad=((np.dot(X.T,(np.dot(X,np.array([theta]).T))-np.array([y]).T)/X.shape[0]).T)[0]
        grad[1:] = grad[1:] + reg*theta[1:]/num_examples 
